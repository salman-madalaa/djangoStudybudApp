...
def getRoom(request):
    rooms = Room.objects.all();
    serializers =  RoomSerializer(rooms) # Note : Serializers are used to convert python objects turn into a Json objects . Many is true because when we pass more than one object we do many=True mandatory.
    return Response(serializers.data)