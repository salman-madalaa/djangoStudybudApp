...
def getAllRooms(request):
    ...
    topics = Topic.objects.all()[0:5]  # getting the all topic objects from database
...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        #    return redirect('Get All Rooms')
    context = {'form': form,'type':type,'topics':topics}
...
def updateRoom(request, id):
    ...
    if request.method == 'POST':
        ...
        #     form.save()
        #     return redirect('Get All Rooms');
    context = {'form': form,'type':type,'topics':topics,'room':room}
    return render(request, 'room/room_form.html', context)
...
def recentlyActivitiesPage(request):
    context={}
    return render(request, 'room/recent_activities.html', context)